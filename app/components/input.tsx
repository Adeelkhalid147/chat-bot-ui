...
interface InputProps {
  onSendMessage: (message: string, files: File[], imageUrls: string[]) => void;
}
...
const AudioTranscription: React.FC<InputProps> = ({ onSendMessage }) => {
  ...
  const handleSendMessage = () => {
    ...
    const imageUrls =
      message.match(/https?:\/\/\S+\.(?:png|jpg|jpeg|gif)/gi) || [];
    if (imageUrls.length > 0) {
      console.log("Sending image URLs:", imageUrls);
    }
    onSendMessage(message, selectedFiles, imageUrls); // Send message, files, and all image URLs
    setMessage(""); // Clear the input field after sending
    setSelectedFiles([]); // Clear the selected files
  };
